...
%   y0_opt           whether to optimize uy0 ([0] 1)
...
%    T_opt - optimal values as a structure
%    T_opt_se - s.e.  as a structure
...
if isnan(kappa)
    T{1}.kappa =10;      T{2}.kappa=0.1;    T{3}.kappa=1000;
else
    T{1}.kappa =kappa;      T{2}.kappa=kappa;    T{3}.kappa=kappa;
end

if isnan(tnd)
    T{1}.tnd =0.35;      T{2}.tnd=0.0;     T{3}.tnd=max(D.rt);
else
...
if opt
    [theta_opt,nlogl] = fmincon(@(theta) dtb_cost_means(theta,P,D,rt_only,0),theta,[],[],[],[],theta_lo,theta_hi,[],options);
...
[nlogl,grad,hess, W]=dtb_cost_means(theta_opt,P,D,rt_only,1);
...
[~,~,~, Wpred]=dtb_cost_means(theta_opt,P,Dopt,rt_only,0);
