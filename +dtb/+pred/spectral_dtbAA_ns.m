function D =  spectral_dtbAA_ns(drift,t,Bup,Blo,y,y0,notabs_flag)
% D =  spectral_dtbAA_ns(drift,t,Bup,Blo,y,y0)
%
% Spectral dtb with nonstationary drift.
...
% Antialiasing, reindexing, nonstationary drift by Yul Kang.
...
% % Nonstationary variance - YK
% % In this case, sig could be a scalar.
% if ~isscalar(sig)
%     % If nonstationary, size(drift) == [ntrial, length(t)]
%     assert(size(drift, 2) == length(t), ...
%         'In nonstationary variance, the number of columns must match length(t)!');
% end 
...
omega=2*pi/range(y)*kk;
E1=repmat(exp(-0.5*dt*omega.^2), [1,nd]); %fft of the normal distribution - scaled suitably by dt
exp_omega1 = exp(-1i.*omega(:,1).*dt);
