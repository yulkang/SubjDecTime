function D = analytic_dtb_sigs(drift,sig,t,Bup,Blo,y0,ny)
% D = analytic_dtb_sigs(drift,sigs,t,Bup,Blo,y0,ny)
%
% Allows multiple sigmas for each coherence.
%
...
% Allow sigma to vary depending on the condition. Scale k and t.
assert(all(sig > 0))
if ~isscalar(sig)
    assert(isvector(sig));
    
    n_drift = length(drift);
    for i_drift = n_drift:-1:1
        D0 = D.
