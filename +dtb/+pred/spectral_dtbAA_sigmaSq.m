function D =  spectral_dtbAA_sigmaSq(drift,t,Bup,Blo,y,y0,sigmaSq,notabs_flag)
...
% % Scale so that the convolution does not change scale
% y_delta = double(y(:) == 0);
% y_delta_conv = max(real(ifft(E1(:,1) .* fft(y_delta))), 0);
% scale_E1 = sum(y_delta_conv);
% E1 = E1 ./ scale_E1;
