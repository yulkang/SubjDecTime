classdef MatFileUnsealed < dynamicprops 
%MatFileUnsealed Save and load parts of variables in MAT-files.
%   MatFileUnsealed objects allow you to load and save parts of variables
%   in a MAT-file. Working with part of a variable requires less memory
%   than working with its entire contents.
%
%   MATOBJ = MatFileUnsealed(FILENAME) constructs an object that can load or save
...
%   include a full or partial path, otherwise MatFileUnsealed searches along the 
%   MATLAB path. If the file does not exist, MATLAB creates the file on
%   the first assignment to a variable.
%
%   MATOBJ = MatFileUnsealed(FILENAME,'Writable',ISWRITABLE) enables or disables
%   write access to the file. ISWRITABLE is logical TRUE (1) or FALSE (0).
%   By default, MatFileUnsealed opens existing files with read-only access, but
...
%             
%    * MatFileUnsealed only supports partial loading and saving for
...
%      with MatFileUnsealed use V7.3 format.
%
%    * MatFileUnsealed does not support linear indexing, or indexing into
...
%      array.
%
%    * You cannot evaluate function handles with a MatFileUnsealed object.
%
%   Methods:
%
...
%      matObj = MatFileUnsealed(myfile,'Writable',true);
%
...
% Copyright 2011 The MathWorks, Inc.
    ...
    properties (SetAccess = private)
        %Properties - Properties that describe and affect the MatFileUnsealed class.
        %
        %   Properties.Source    - Fully qualified path to the file.
        %   Properties.Writable  - Whether to allow saving to the file. Logical
        %                          TRUE (1) or FALSE (0).
...
%   Example:
%
%      durer = MatFileUnsealed('durer.mat');
%      who(durer)
%
%   See also MatFileUnsealed, whos, size, clear, clearvars, save, load.
            try
...
%
%   Input Arguments:
%      MATOBJ     Object created by calling the MatFileUnsealed function.
%
...
%      nDimsX = length(sizeX)
% 
%   See also MatFileUnsealed, who, size, clear, clearvars, save, load.
            try
...
%
%      mfObj = MatFileUnsealed('temp.mat','Writable',true);
%      mfObj.X = rand(2,3,4);
...
%
%   See also MatFileUnsealed, whos, who, save load, length, ndims, numel.
            narginchk(1,3);
            try
        ...
        function obj = MatFileUnsealed(Source, varargin)
            
            narginchk(1, inf);
          
            obj.Properties = matfileUnsealed.Properties(resolveSource(Source));
        ...
        function varargout = subsref(obj, indexingStruct)
            ...
            try
                ...
                caughtException = overrideCaughtMessages(caughtException, varName);
                ...
                if strcmp(varName,'Properties')
                    ...
                    switch length(indexingStruct) 
                        ...
                        case 2
                            ...
                            varSubset = createMetaDescriptionOfIndexing(varName, varInfo, indexingStruct);
                            if obj.Properties.SupportsPartialAccess
                                internal.matlab.language.partialSave(obj.Properties.Source, value, varSubset);
                            else
...
function sourceName = resolveSource(sourceName)
    try
        sourceName = internal.matlab.language.findFullMATFilename(sourceName);
    catch caughtException
        throwAsCaller(caughtException);
    end
...
function varSubset = createMetaDescriptionOfIndexing(varName, varInfo, indexingStruct) 
    ...
    if iscellstr(indexExpression) && ~any(strcmp(indexExpression,':'))
        ...
        % indexing.
        subsets = internal.matlab.language.Subset(indexingStruct(depthOfIndexing).type, {indexExpression});
    else
        ...
        subsets = internal.matlab.language.Subset(indexingStruct(depthOfIndexing).type, boundsTriplet);
    end
    varSubset = internal.matlab.language.VariableSubset(varName, subsets);
end

function validateFirstArgIsObj(obj, methodName)
    if ~isa(obj, 'MatFileUnsealed') % 'MatFileUnsealed')
        % First argument must be a MatFileUnsealed
...
function caughtException = overrideCaughtMessages(caughtException, varName)
    if strcmp(caughtException.identifier,'MATLAB:Subset:ImproperIndexCell')
        caughtException = MException('MATLAB:MatFileUnsealed:NeedsAllDims',...
            'To index into ''%s'' using a MatFileUnsealed object, specify indices for all dimensions.',varName);
    end
end
