classdef PsyEyeRaw < PsyMouse
    % Some parts copied/derived from Roozbeh Kiani's PsychLib, esp. initEyelink.m
...
... %            ,'binocular_enabled',                'NO'    ...
        ...
        raw_mode   = 'N'; % N (none) | P_CR (pupil - corneal reflex) | P | CR
        log_xyPixRaw = false;
        ...
        calibGain = [1 1]'/500; % xyPixRaw * calibGain + calibPix = xyPix.
	...
	methods
        %% Before experiment
        function Eye = PsyEyeRaw(cScr, varargin)
            % Eye = PsyEyeRaw([Scr], ['optName1', opt1, ...]);
            ...
            Eye.initLogEntries('prop2', {'xyPix', 'pupil', 'sampledAbsSec'}, 'absSec', ...
                              {single([nan;nan]), single(nan), nan}, Eye.maxNSampEye);
                          
            % xyPixRaw is not logged when calibration 
        ...
        function res = initEL(Eye)
            ...
            if ~strcmp(Eye.raw_mode, 'N')
                Eye.initConf({
                    'link_sample_data', 'LEFT,RIGHT,GAZE,AREA,GAZERES,HREF,PUPIL,STATUS,INPUT,HMARKER'
                    'inputword_is_window', 'on'
%                     'file_sample_data', 'LEFT,RIGHT,GAZE,AREA,GAZERES,HREF,PUPIL,STATUS,INPUT,HMARKER'
                    });
            end
...
%                     EyelinkDoDriftCorrection(Eye.el);   
        ...
        function get(Eye) % cXYDeg = get(Eye)
            ...
            whichEye  = Eye.whichEyeVec(2);
            centerPix = Eye.centerPix;
            save_samp = Eye.saveDat.samp;
            cSamp = nan(31,1);
            
            to_log = {'xyPix', 'pupil', 'sampledAbsSec'};
            if Eye.log_xyPixRaw
                to_log = [to_log, {'xyPixRaw'}];
            end
            
            get_raw = ~strcmp(Eye.raw_mode, 'N');

            if get_raw
                cSamp = Eyelink('NewestFloatSample');
                
                xyPixRaw = [cSamp.px(whichEye+1); cSamp.py(whichEye+1)];
                
                xyPix = (xyPixRaw - centerPix + calibPix) .* calibGain + centerPix;
                pupil = cSamp.pa(whichEye+1);
                cAbsSec = GetSecs;
                
                Eye.xyPixRaw = xyPixRaw;
                Eye.pupil = pupil;
                Eye.sampledAbsSec = cAbsSec;
                
                % Log with EyeLink time, shifted to match GetSecs.
                addLog(Eye, to_log, ...
                       cSamp.time./1000 + Eye.samp2GetSecs);

                Eye.samp = cSamp; % DEBUG
                ...
                while ~isempty(cSamp) % ~drained
                    ...
                    [cSamp, ~, ~] = Eyelink('GetQueuedData'); % [cSamp, cEvt, drained] = Eyelink('GetQueuedData');
                    ...
                    if ~isempty(cSamp)
                        ...
                        %
                        % NOTE: Only the right eye is used if both eyes are sampled,
                        %       for both xyPix and pupil.
                        % See also: Eyelink GetQueuedData?
                        % 
                        % - Eye.calibPix: Currently only handles one calibration target.
                        % See PsyEye.calibEL.
                        %
                        xyPixRaw = cSamp([14, 16] + whichEye, :);

                        % Offset should center the gaze. Gain should change the
                        % eccentricity.
                        xyPix(1,:) = (xyPixRaw(1,:) + calibPix(1)) * calibGain(1) + centerPix(1);
                        xyPix(2,:) = (xyPixRaw(2,:) + calibPix(2)) * calibGain(2) + centerPix(2);

                        pupil = cSamp(12 + whichEye,:);

                        Eye.xyPix = xyPix;
                        Eye.pupil = cSamp(12 + whichEye,:);

                        % sampledAbsSec: Contents (v_) are retrieval GetSecs,
                        %                timestamps (t_) are the tracker time.
                        Eye.sampledAbsSec = repmat(cAbsSec, [1 nSamp]);

                        % Log with EyeLink time, shifted to match GetSecs.
                        addLog(Eye, to_log, ...
                               cSamp(1,:)./1000 + Eye.samp2GetSecs);
                        ...
                        if save_samp
                            ...
                            addLog(Eye, {'samp'}, Eye.sampledAbsSec, {cSamp});
                        end
                    end
                end
            end
        ...
        function plotRaw(Eye, varargin)
            S = varargin2S(varargin, {
                });
            
            t = Eye.t('xyPixRaw')'; t = t - t(1);
            v = Eye.v('xyPixRaw')';
            
            subplot(2,1,1);
            plot(t, v(:,1));
            xlabel('t (s)'); ylabel('x (raw)');
            
            subplot(2,1,2);
            plot(t, v(:,2));
            xlabel('t (s)'); ylabel('y (raw)');            
        end
