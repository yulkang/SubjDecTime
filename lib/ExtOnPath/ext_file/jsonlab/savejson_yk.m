function json=savejson_yk(rootname,obj,varargin)
%
...
%
% YK: 
% In the original jsonlab 1.0 alpha, struct('a',{{}}) was incorrectly saved as
% struct('a',[]). I appended {'', ''} to all cell arrays in savejson_yk and
% removed them back in loadjson_yk to work around it.
% This works fine for a struct with fields of strings and cell arrays of strings,
% but fails for other cases (e.g., cell array as the root object).
...
item = [item(:); {'';''}]; % Add two entries to workaround type inconsistency
