...
function [name, package] = module(package,mode)
...
while true
    packsep = find(package == '.');
    go_up   = find(package == '^');
    
    if isempty(go_up), break; end
    
    bef_up = packsep(find(packsep < go_up(1), 2, 'last'));
    aft_up = packsep(find(packsep > go_up(1), 1, 'first'));
    
    if isempty(bef_up) && isempty(aft_up)
        package = '';
        break;
    else
        if isempty(aft_up), aft_up = length(package) + 1; end
        switch length(bef_up) 
            case 0
                bef_up = [1 1];
            case 1
                bef_up = [1, bef_up]; %#ok<AGROW>
        end

        package = [package(1:(bef_up(1)-1)), '.', package((aft_up+1):end)];
    end
end

if package(1) == '.'
    package(1:(find(package ~= '.', 1, 'first') - 1)) = '';
end
...
for n=1:numel(file)    
    ...
    name.(function_name)=str2func(str_bridge('.', package, function_name));
end
