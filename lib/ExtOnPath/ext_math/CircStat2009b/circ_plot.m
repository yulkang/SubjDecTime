...
switch format
  ...
  case 'hist'
    ...
    [t,r] = rose(alpha,x);
    if varargin{2}
      polar(t,r/(sum(r)/2),formats) % modified from sum(r) to (sum(r)/2), to make them sum to 1. (HR Kang)
      mr = max(r/(sum(r)/2)); % modified from sum(r) to (sum(r)/2), to make them sum to 1. (HR Kang)
    else
      polar(t,r,formats)
      mr = max(r);
    end
