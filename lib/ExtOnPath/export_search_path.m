function export_search_path
% Exports your current search path, minus the matlab root directories,
% into a path file named "__matlab__search__path__.mat"
% That path file will be stored in the current directory, along with a
% function that is created on the fly, called importpath.m
% 
% You can them start a new version of MATLAB, cd to the chosen directory,
% and execute the function import_search_path, which will update a new MATLAB
% release to have the corresponding path directories added to your search
% path.

% grab the current path
p = path;

% split it apart into a cell array of containing each distinct directory
% on the searchpath, using pathsep as the splitting character.
pdirs = allwords(p,pathsep);

% get the current matlabroot directory
mrdir = matlabroot;

% which directories were under matlabroot?
mrbool = strncmpi(pdirs,mrdir,numel(mrdir));

% index of the first matlabroot sub-directory
mr1 = find(mrbool,1,'first');

% which directories to be exported were on top of the path, appearing
% before the matlabroot dirs?
expdirs_top = find(~mrbool);
expdirs_top(expdirs_top > mr1) = [];

% which directories to be exported will be put at the end of the path,
% appearing after the matlabroot dirs?
expdirs_bottom = find(~mrbool);
expdirs_bottom(expdirs_top < mr1) = [];

% create the lists of directories to be exported as a struct
exported_directory_list.top = pdirs(expdirs_top);
exported_directory_list.bottom = pdirs(expdirs_bottom);

% and save the struct to a mat file, in the current directory. 
save __matlab__search__path__ exported_directory_list

% finally, create the import_search_path m-file on the fly, saved to an m-file
% in the current directory. That file is given here as a cell array:
import_search_path_mfile = {'function import_search_path', ...
  '% imports the searth path directories into a newly downloaded MATLAB release.' ...
  '% assumes that the MATLAB release will have its path set to the default path.' ...
  'load __matlab__search__path__.mat' ...
  '% append directories to the bottom in order of their appearance in the old' ...
  '% search path' ...
  'nb = numel(exported_directory_list.bottom);' ...
  'if nb > 0' ...
  '  for k = 1:nb' ...
  '    addpath(exported_directory_list.bottom{k},''-end'')' ...
  '  end' ...
  'end' ...
  '% append directories to the top in reverse order of their appearance in' ...
  '% the old search path' ...
  'nt = numel(exported_directory_list.top);' ...
  'if nt > 0' ...
  '  for k = nt:-1:1' ...
  '    addpath(exported_directory_list.top{k},''-begin'')' ...
  '  end' ...
  'end'};

% open the import_search_path m-file, write, and close it
FID = fopen([pwd,filesep,'import_search_path.m'],'w');
for k = 1:numel(import_search_path_mfile)
  m = numel(import_search_path_mfile);
  fprintf(FID,'%s\n',import_search_path_mfile{k});
end
fclose(FID);
