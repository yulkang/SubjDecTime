...
% test  Testing the Localog package.
%
% See also Localog.name, Localog, PsyLib
%
...
disp('location of the ''root'' or ''base_caller'' program that calls Localog.name.');
disp('That is, if test1.m calls test2.m, and test2.m calls Localog.name,');
...
disp(' ');
disp('''parallel'' is the default for Localog.name and recommended');
...
disp('    by giving Localog.name options ''code_base'' and ''data_base''.');
disp(' ');
...
disp('    by giving Localog.name option ''data_subdir''.');
disp(' ');
...
if ~Localog.is_versioned
    error('Make sure to put +Localog/ folder within a Git repository!');
end
...
disp('Localog.name automatically detects changes and asks if to commit.');
disp('If you choose to commit, it will also ask for a commit message.');
disp('Then it will add & commit all uncommitted changes.');
disp(' ');
if Localog.get_status % if any uncommitted change is detected,
    disp('Uncommitted changes detected!');
    disp('Do you want to let Localog.name commit for you? It will ask for the commit message.');
    disp('Alternatively, you can choose to stop and commit for yourself.');
    disp(' ');
    if ~inputYN_def('Type ENTER or ''y'' to let Localog.name commit, and ''n'' to stop. (Y/n) ', true);
        disp(' ');
        disp('You chose to stop and add/commit changes for yourself!');
        disp('Run Localog.test again after you do so.');
        return;
    end
else
...
if inputYN_def('Save diary, data and figure (Y/n)?  ', true)
    % Get comment. 
    % If 'comment' argument is ommitted, Localog.name will ask for it by default,
    ...
    disp('which I will ask you separately if you commit via Localog.name.');
    disp(' ');
    comment = input('Comment (added to the new file''s name): ', 's');
    disp(' ');
    
    disp('Note that, if your repository has changes,');
    disp('Localog.name will ask for a separate commit message.');
    disp(' ');

    disp('Saving diary. Logging happens on call to Localog.name.');
    disp('In real use, put diary() before you print important contents.');
    diary_file = Localog.name([subj '/diary'], kind, '.txt', comment, {}, 'scheme', scheme);
    diary(diary_file);
    disp(' ');
    
    disp('Saving the data.');
    dat_file = Localog.name([subj '/mat'], kind, '.mat', comment, {}, 'scheme', scheme);
    save(dat_file, 'r');
    disp(' ');

    disp('Saving the plot.');
    [fig_file, log_data] = ...
        Localog.name([subj '/fig'], kind, '.eps', comment, {dat_file}, 'scheme', scheme);
    ...
    disp('which is the second output from Localog.name:');
    disp(log_data);
    disp(' ');
    ...
    disp(' ');
    disp('=====');
    disp('This concludes the tutorial. See help Localog.name for details. Enjoy!');
    disp('=====');
    disp(' ');
