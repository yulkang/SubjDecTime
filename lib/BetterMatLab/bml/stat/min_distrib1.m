function [p_min, p_1st] = min_distrib1(p1, p2, sum_equals)
...
siz1 = size(p1);
p = cat(2, reshape(p1, siz1(1), 1, []), reshape(p2, siz1(1), 1, []));

[p_min, p_1st] = min_distrib(p, sum_equals);
p_min = reshape(p_min, siz1);
p_1st = {reshape(p_1st(:,1,:), siz1), reshape(p_1st(:,2,:), siz1)};
