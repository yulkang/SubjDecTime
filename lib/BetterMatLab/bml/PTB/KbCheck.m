...
if ~IsWin || (IsWin && ~isempty(deviceNumber))
    if ~isempty(deviceNumber)
        ...
        % Query primary keyboard: % debug
        for ii = [5 4 3 2 1]
            try
                [keyIsDown, secs, keyCode]= PsychHID('KbCheck', ii, ptb_kbcheck_enabledKeys);
                break;
            catch
                warning('Device %d is not a keyboard.\n', ...
                    ii);
            end
        end
    end
